...
def executar_testes_e_salvar_resultados():
    with open("../testes/entrada.txt", "r") as file:
        ...
        for idx, line in enumerate(file, start=1):
            ...
            for _ in range(5):
                tempo_guloso = timeit.timeit(lambda: particao_gulosa(nums),
                                             number=1)
                tempo_backtracking = timeit.timeit(lambda:
                                                   pode_particionar(nums),
                                                   number=1)
            ...
            media_backtracking = sum(
                tempos_backtracking) / len(tempos_backtracking
                                           )

            conjunto1_guloso, conjunto2_guloso = particao_gulosa(nums)
            validade_guloso = "Sim" if sum(conjunto1_guloso) == sum(
                conjunto2_guloso) else "Não"
            _, conjunto1_backtracking, conjunto2_backtracking\
                = pode_particionar(nums)
            validade_backtracking = "Sim" if sum(
                conjunto1_backtracking) == sum(
                conjunto2_backtracking) else "Não"

            resultados.append({
                'media_guloso': media_guloso,
                'media_backtracking': media_backtracking,
                'qualidade_guloso': abs(sum(conjunto1_guloso) - sum(
                    conjunto2_guloso)),
                'qualidade_backtracking': abs(
                    sum(conjunto1_backtracking) - sum(conjunto2_backtracking)),
        ...
        df_resultados.to_csv('resultados/resultados_comparacao.csv',
                             index=False)
        ...
        axs[0].plot(df_resultados['comprimento_vetor'],
                    df_resultados['media_backtracking'],
                    label='Backtracking (Exato)', marker='o')
        axs[0].plot(df_resultados['comprimento_vetor'],
                    df_resultados['media_guloso'],
        ...
        axs[1].plot(df_resultados['comprimento_vetor'],
                    df_resultados['qualidade_backtracking'],
                    label='Backtracking (Exato)', marker='o')
        axs[1].plot(df_resultados['comprimento_vetor'],
                    df_resultados['qualidade_guloso'],
                    label='Heurística Gulosa',
                    marker='o')
